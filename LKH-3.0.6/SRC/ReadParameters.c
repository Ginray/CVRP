...
void ReadParameters()
{
    ...
    while ((Line = ReadLine(ParameterFile))) {
        ...
        if (!strcmp(Keyword, "ASCENT_CANDIDATES")) {
            ...
            continue;
        }else if (!strcmp(Keyword, "CLUSTERING_K")) {
            CLUSTERING_K = 0;
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%d", &CLUSTERING_K))
                eprintf("CLUSTERING_K: integer expected");
        } else if (!strcmp(Keyword, "CLUSTERING_FILENAME")) {
            if (!(Clustering_file_name = GetFileName(0)))
                eprintf("CLUSTERING_FILENAME: string expected");
