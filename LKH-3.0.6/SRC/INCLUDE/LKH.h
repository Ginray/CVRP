...
int OldCapacityList[1000];  // 从大到小的Capacity值
int salesmen_temp; //多容量下的salesmen数量，就不是通过计算得出的了，而是给出的需求值进行相加。
int CLUSTERING_K ; // 聚类的K的值
char *Clustering_file_name ; //聚类的参数文件

struct Node {
    Node * next_unset ; /* 如果这是一个未在rour中的点，这个指针用于查找下一个未在tour中的点 */
    int Capacity_ID ; /* 所属的tour的Capacity_ID，从1开始 */
    int Capacity;  /* 所属的tour的Capacity */
...
*/
    // MaxCapacityNum = 1000;
    int CapacityList[1000];  // 不同类型的车辆的容量
    int CapacityNum [1000];  // 不同类型的车辆的个数
    int CapacityNumberSum ;  // 车辆总数 
    int CapacityNumberStart; // 起始的车辆ID(从0开始)
/*
    END
*/
