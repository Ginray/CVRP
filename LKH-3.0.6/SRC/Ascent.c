...
GainType Ascent()
{
    ...
    int T, Period, P, InitialPhase, BestNorm;  //T / Period : 控制迭代次数
  ...
  Start:
     ...
     */
    if (!Subgradient || !Norm)  //Norm == 0 : 找到了一个true optimum
        return W;
    ...
    if (MaxCandidates > 0) {
        ...
        if (CandidateSetType != DELAUNAY && CandidateSetType != POPMUSIC)
            GenerateCandidates(AscentCandidates, MaxAlpha, 1);           // 找到了一些候选点
        else {
    ...
    BestW = W0 = W;
    BestNorm = Norm;
    InitialPhase = 1;  //  subradient 操作开始，用于优化Pi值，Pi值和degree相关
