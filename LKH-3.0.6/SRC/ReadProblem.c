...
static void Read_CAPACITY_LIST(void);
static void Read_CAPACITY_LIST_NUM(void);
...
void ReadProblem()
{
    ...
    while ((Line = ReadLine(ProblemFile))) {
        ...
        if (!strcmp(Keyword, "COMMENT"));
            ...
            Read_CAPACITY();
        else if (!strcmp(Keyword, "CAPACITY_LIST"))
            Read_CAPACITY_LIST();
        else if (!strcmp(Keyword, "CAPACITY_LIST_NUM"))
            Read_CAPACITY_LIST_NUM();
    ...
    if (ProblemType == CVRP) {
        ...
        MinSalesmen = salesmen_temp; // new feature here
        if (Salesmen == 1) {
    ...
    if (Salesmen > 1) {
        ...
        MTSP2TSP();  //处理了Node的rank = distance  ,同时把DimensionSaved变更为 Node数+salesman数量 ，Nodeset同时扩大为Node数+salesman数量的大小
    }
...
static void Read_CAPACITY_LIST()
{
    char *Token = strtok(0, Delimiters);
    int temp_num = 0;

    if (!Token){
        eprintf("Read_CAPACITY_LIST: Integer expected");
        return ;
    }
    char *temp_Token = NULL;
    temp_Token = strtok(Token,",");
    while(temp_Token){
        sscanf(temp_Token, "%d", &CapacityList[temp_num]) ; 
        temp_Token = strtok(NULL,",");
        temp_num++;
    }
    CapacityNumberSum = temp_num;
    CapacityNumberStart = 0 ;
}


static void Read_CAPACITY_LIST_NUM()
{
    char *Token = strtok(0, Delimiters);
    int temp_num = 0;

    if (!Token){
        eprintf("Read_CAPACITY_LIST_NUM: Integer expected");
        return ;
    }
    char *temp_Token = NULL;
    temp_Token = strtok(Token,",");
    salesmen_temp = 0;
    while(temp_Token){
        sscanf(temp_Token, "%d", &CapacityNum[temp_num]) ; 
        salesmen_temp += CapacityNum[temp_num];
        temp_Token = strtok(NULL,",");
        temp_num++;
    }
    assert(temp_num == CapacityNumberSum); // 车的数量需要相等
}
