...
int main(int argc, char *argv[])
{
    ...
    MergeWithTour = Recombination == IPT ? MergeWithTourIPT :
        MergeWithTourGPX2;
    ReadProblem(); // 构造了Node-list 
    printf("CLUSTERING_K = %d\n",CLUSTERING_K);
    if (SubproblemSize > 0) {   
    ...
    AllocateStructures();  //构造了 two-level-tree 以及需要的参数

    if (ProblemType == VRPB || ProblemType == VRPBTW)
        VRPB_Reduce();
    if (ProblemType == PDPTW)
        PDPTW_Reduce();
    CreateCandidateSet();   // 包括1-tree构造，Ascent等操作
    InitializeStatistics();

    if (Norm != 0 || Penalty) {
        printf("after ascent , Penalty = %d\n",Penalty);
        Norm = 9999;
    ...
    for (Run = 1; Run <= 1; Run++) {
        ...
        if (MaxPopulationSize > 1 && !TSPTW_Makespan) {   // 遗传算法
            /* Genetic algorithm */
            int i;
